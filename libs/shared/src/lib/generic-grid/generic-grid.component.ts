import { Component, Input, Output, EventEmitter, OnInit, ViewChild, AfterViewInit, OnDestroy, ElementRef } from '@angular/core';
...
import { GridConfigurationsDrawerComponent } from './grid-configurations-drawer/grid-configurations-drawer.component';
...
@Component({
  ...
  imports: [
    ...
    GridConfigurationsDrawerComponent
  ],
...
export class GenericGridComponent<T> implements OnInit, AfterViewInit, OnDestroy {  
  ...
  get getGridHeaderBackgroundColor(): string {
    return this.columns[0].headerBackgroundColor || 'transparent';
  }
  get getGridHeaderTextColor(): string {
    return this.columns[0].headerTextColor || 'transparent';
  }

  getGridCellBackgroundColor(index:number): string {
    return this.columns[index].cellBackgroundColor || 'transparent';
  }
  getGridCellTextColor(index:number): string {
    return this.columns[index].cellTextColor || 'transparent';
  }
  
  get getGridCellActionsBackgroundColor(): string {
    return this.columns[0].cellActionsBackgroundColor || 'transparent';
  }
  ...
  constructor(
    private _gridService: GridService<T>, 
  ...
  onAction(action: EnumActionsType | EnumActions, element: T): void {
    switch (action) {
  ...
  private _subcriptionData(): void {
    this._subscriptions.push(
      this._gridService.getData().subscribe((data: T[]) => {
  ...
  private _subscriptionColumns(): void {
    this._subscriptions.push(
      this._gridService.getColumns().subscribe((cols: GridColumn<T>[]) => {
        this.columns = cols;
  ...
  private _subscriptionActions(): void {
    this._subscriptions.push(
      this._gridService.getActions().subscribe((actions: Action[]) => {
