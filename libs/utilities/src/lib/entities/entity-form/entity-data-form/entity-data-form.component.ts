...
export class EntityDataFormComponent implements OnInit, ISectionForm {
  ...
  @Input() set entity(entity: Entity | undefined) {
    this._entity = entity;
    if (entity) { this._updateEntity(entity); this.isLoading = false; }
  }
  ...
  get data(): Entity { return this._entityFormToEntity(); }
  get modified(): boolean { return this.entityForm.dirty; }
  get valid(): boolean { return this.entityForm.valid; }
  ...
  ngOnInit(): void {
    this._loadHiddenFields().subscribe(() => {
      if (this._entity) { this._updateEntity(this._entity); this.isLoading = false; }
    });
  }
  ...
  private _createForm(): void {
    ...
    this.entityForm.valueChanges.subscribe(() => { this.dataChange.emit(); });
  }

  private _entityFormToEntity(): Entity {
    this._entity = { ...this._entity, ...this.entityForm.value } as Entity;
    return this._entity!;
  }
  ...
  private _updateEntity(entity: Entity): void {
    ...
    if (entity.objectMode !== EnumObjectMode.READONLY) { this.entityForm.enable(); }
