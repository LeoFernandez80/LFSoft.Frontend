...
export class FormValidationsDirective implements OnInit {
  ...
  @Input('appFormValidations')  formGroup!: FormGroup;

   @Input() errorMessages: { [key: string]: string } = {};

  constructor(private el: ElementRef, private renderer: Renderer2, private _translationService: TranslationService) {}

  ngOnInit(): void {
    if (!this.formGroup) {
      console.error('appFormErrorHover requires a FormGroup');
      return;
    }
