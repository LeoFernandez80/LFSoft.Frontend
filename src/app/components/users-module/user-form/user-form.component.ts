...
import { PersonFormComponent } from '../../persons-module/person-form/person-form.component';
...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, RouterOutlet,GenericFormComponent, 
    ...
    SkeletonDirective, MatButtonModule, PersonFormComponent],
...
export class UserFormComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder, private _userService: UserService, private _route: ActivatedRoute, 
  ...
  private _loadData() {
    ...
    this._loadParams().subscribe(() => {                  
      switch (this._operation) {
        case 'open':          
          this._editUser(this.userId).subscribe(user => {;
  ...
  private _editUser(id: number): Observable<User> {
  ...
  private _save(): void {
    try {
      ...
      if (!updatedUser.id){
        this._userService.addUser(updatedUser);
      } else{
